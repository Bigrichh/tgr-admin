<template>
  <section class="w-full min-h-screen h-fit flex flex-col pb-[240px]">
    ...
    >
      <h1 class="text-[16px] font-extrabold uppercase leading-[150%]">Dashboard</h1>
    ...
    <div class="w-full p-[24px] flex flex-col lg:flex-row gap-[16px]">
      ...
      <div class="stat-card">
        <div class="w-full">
          <div class="w-full flex items-center justify-between">
            <h2 class="text-[14px] font-semibold leading-[140%]">Total Orders</h2>
            <div class="size-[16px]">
              <img
                class="size-full object-cover"
                src="../assets/svgs/navbar/orders.svg"
                alt="orders-icon"
              />
            </div>
          </div>
      ...
      <div class="stat-card">
        <div class="w-full">
          <div class="w-full flex items-center justify-between">
            <h2 class="text-[14px] font-semibold leading-[140%]">Pending Orders</h2>
            <div class="size-[16px]">
              <img
                class="size-full object-cover"
                src="../assets/svgs/navbar/orders.svg"
      ...
      <div class="stat-card">
        <div class="w-full">
          <div class="w-full flex items-center justify-between">
            <h2 class="text-[14px] font-semibold leading-[140%]">Orders This Month</h2>
            <div class="size-[16px]">
              <img
                class="size-full object-cover"
                src="../assets/svgs/navbar/orders.svg"
      ...
      <div class="stat-card">
        <div class="w-full">
          <div class="w-full flex items-center justify-between">
            <h2 class="text-[14px] font-semibold leading-[140%]">Orders Today</h2>
            <div class="size-[16px]">
              <img
                class="size-full object-cover"
                src="../assets/svgs/navbar/orders.svg"
    ...
    <!-- Big Status cards -->
    <div class="w-full flex flex-col lg:flex-row gap-[16px] px-[24px] pb-[24px]">
      <!-- Card 1 -->
      <div class="w-full big-stat-card">
        <!-- Loader -->
        <div
          v-if="!contentStore.ordersLoaded"
          class="w-full flex items-center justify-center min-h-[359px]"
        >
          <p class="text-[14px]">Loading sales stats...</p>
        </div>
        <div v-else class="w-full flex flex-col">
          <!-- Header -->
          <div class="w-full flex items-start justify-between gap-[40px] md:gap-0 p-[24px]">
            <div class="w-full flex flex-col gap-[8px]">
              <h2 class="text-[16px] font-semibold leading-[100%]">Recent Sales</h2>
              <p class="text-[14px] opacity-50 leading-[110%]">
                You've made <span>{{ ordersThisMonth }} sales this month</span>
              </p>
            </div>
            <div class="w-full flex flex-col gap-[8px] text-right">
              <h2 class="text-[16px] font-semibold leading-[100%]">
                GHS {{ totalSalesThisMonth }}
              </h2>
              <p class="text-[14px] opacity-50 leading-[110%]">Total sales this month</p>
            </div>
          </div>
          <!-- Recent Sales -->
          <div
            class="w-full flex flex-col gap-[16px] items-start justify-between px-[24px] pb-[24px]"
          >
            <div
              v-for="order in contentStore.orders.slice(0, 3)"
              class="w-full hover:bg-[#f4f4f5] p-[8px]"
            >
              <router-link :to="{ name: 'order-details', params: { id: order.orderId } }">
                <!-- Customer Card -->
                <div
                  class="w-full flex flex-row items-center md:items-center justify-between gap-[16px]"
                >
                  <div
                    class="size-[45px] bg-[#F4F4F5] rounded-full flex items-center justify-center"
                  >
                    <p class="uppercase">
                      {{
                        order.customerName
                  ...
                  <div
                    class="w-full flex flex-col md:flex-row items-start md:items-center justify-between gap-[8px] md:gap-0"
                  >
                    <div class="flex flex-col gap-[4px]">
                      <p class="text-[14px] font-medium leading-[100%]">
                        {{ order.customerName }}
                      </p>
                      <p class="text-[14px] opacity-50 leading-[120%]">
                        {{ order.customerEmail }}
                      </p>
                    </div>
                    <p class="text-[14px] font-medium">+GHS {{ order.totalCost }}</p>
                  </div>
                </div>
              </router-link>
            </div>

            <router-link class="btn-alt px-[16px] py-[8px] self-end" :to="{ name: 'orders' }">
              <div class="size-[16px]">
                <img
                  class="size-full object-cover"
                  src="../assets/svgs/angle-arrow.svg"
                  alt="angle-arrow-icon"
                />
              </div>
              <p>View All Orders</p>
            </router-link>
      ...
      <div class="w-full big-stat-card">
        ...
        <div
          ...
          class="w-full flex items-center justify-center min-h-[359px]"
        >
          <p class="text-[14px]">Loading post information...</p>
        </div>
        <div v-else class="w-full flex flex-col">
          <!-- Header -->
          <div class="w-full flex flex-col items-start gap-[8px] p-[24px]">
            <h2 class="text-[16px] font-semibold leading-[100%]">Recent Blog Posts</h2>
            <p class="text-[14px] opacity-50 leading-[110%]">These are your most recent posts</p>
          </div>
          <!-- Recent Posts -->
          <div
            class="w-full flex flex-col gap-[16px] items-start justify-between px-[24px] pb-[24px]"
          >
            <div
              v-for="post in contentStore.blogPosts.slice(0, 3)"
              class="w-full hover:bg-[#f4f4f5] p-[8px]"
            >
              <router-link :to="{ name: 'edit-post', params: { id: post.id } }">
                <!-- Post Card -->
                <div class="w-full flex items-center justify-between">
                  <div class="w-fit flex items-center gap-[16px]">
                    <div
                      class="size-[45px] bg-[#F4F4F5] rounded-full flex items-center justify-center"
                    >
                      <p class="uppercase">
                    ...
                    <div class="flex flex-col gap-[4px]">
                      <p class="text-[14px] font-semibold leading-[100%]">
                        {{ post.title }}
                      </p>
                      <p class="text-[14px] opacity-50 leading-[120%]">
            ...
            <router-link class="btn-alt px-[16px] py-[8px] self-end" :to="{ name: 'blog' }">
              <div class="size-[16px]">
                <img
                  class="size-full object-cover"
                  src="../assets/svgs/angle-arrow.svg"
    ...
    <div class="w-full flex flex-col gap-[24px]">
      ...
      >
        <h2 class="text-[16px] font-semibold leading-[150%]">Pending Orders</h2>
        <!-- Search -->
      ...
      <div class="w-screen h-fit lg:w-full overflow-x-scroll lg:overflow-visible px-[24px]">
        ...
        <div class="w-full lg:w-full h-fit flex flex-col">
            ...
            <tbody>
              ...
              >
                ...
                <td class="!w-[50px] !flex !items-center !justify-center !relative">
                  <!-- Actions Icon -->
                  <div
                    @click="handleActions(order.orderId, $event)"
                  ...
                  <Teleport to="body">
                    <div
                      v-show="activeActionsId === order.orderId"
                      :style="{
          ...
          <div class="!w-full text-[14px] leading-[150%] text-center">
            ...
            <div v-else-if="pendingOrders.length === 0" class="w-full !px-[16px] !py-[10px]">
              No orders available.
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    pendingOrders() {
      const pendingOrders = this.contentStore.orders.filter(
    ...
    totalSalesThisMonth() {
      ...
      return this.contentStore.orders
        .filter((order) => {
          const orderDate = order.orderedAt?.toDate
  ...
  methods: {
    ...
    handleActions(orderId, event) {
      // Toggle open/close
      this.activeActionsId = this.activeActionsId === orderId ? null : orderId
