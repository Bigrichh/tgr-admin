          ...
          <div class="w-full flex items-center justify-between">
            ...
            <div class="size-[16px]">
              <img
                ...
                src="../../../assets/svgs/navbar/ourgentlemen.svg"
                alt="our-gentlemen-icon"
              />
            </div>
          </div>
          <div class="text-[24px] font-extrabold">
            {{ contentStore.ourGentlemen.length }}
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="w-full flex flex-col gap-[24px]">
      ...
      >
        <div class="w-full lg:w-fit flex items-center justify-between">
          <h2 class="text-[16px] font-semibold leading-[150%]">Our Gentlemen</h2>
          <!-- Add New Gentleman Button - Mobile -->
          <router-link
            class="flex lg:!hidden btn-prim px-[16px] py-[8px]"
            :to="{ name: 'add-new-gentleman' }"
            >Add New Gentleman</router-link
          >
        </div>
        <div class="w-full lg:w-fit flex gap-[8px]">
          ...
          <div class="relative w-full lg:w-[300px]">
            ...
            <input
              ...
              name="search"
              v-model="search"
              placeholder="Search for a gentleman"
              autocomplete="off"
            />
          </div>
          <!-- Add New Gentleman Button - Desktop -->
          <router-link
      ...
      >
        ...
        <div class="w-full lg:w-full h-fit flex flex-col">
          ...
          >
            ...
            <tbody>
              <tr
                ...
                v-for="(gentleman, index) in filteredGentlemen"
                :key="gentleman.id"
              >
                <td class="!px-[16px] !py-[10px] !w-[50px]">{{ index + 1 }}</td>
                <td class="!px-[16px] !py-[10px]">{{ gentleman.name }}</td>
                <td class="!px-[16px] !py-[10px]">{{ gentleman.igHandle }}</td>
                <td class="!px-[16px] !py-[10px]">
                  {{ gentleman.uploadedBy }}
                </td>
                <td class="!px-[16px] !py-[10px]">
                  {{ formatDate(gentleman.createdAt) }}
                </td>
                <td
                ...
                >
                  <!-- Actions Icon -->
                  <div
                    @click="handleActions(gentleman.id, $event)"
                  ...
                  <Teleport to="body">
                    <div
                      v-show="activeActionsId === gentleman.id"
                      :style="{
                    ...
                    >
                      ...
                      >
                        ...
                        <div
                          ...
                          @click="handleActions(gentleman.id)"
                        >
                          <img
                      ...
                      <!-- Edit -->
                      <div
                        @click="editGentleman(gentleman.id)"
                      ...
                      <!-- Delete -->
                      <div
                        @click="deleteGentleman(gentleman.id, gentleman.gentlemansPhotoUrl)"
                        class="w-full flex items-center gap-[8px] px-[16px] py-[8px] text-[13px] font-semibold leading-[150%] opacity-50 hover:opacity-100 hover:cursor-pointer border-b-[1px] border-b-[#e4e4e7]"
                      >
                        <div class="size-[16px]">
                          <img
          ...
          <div class="!w-full text-[14px] leading-[150%] text-center">
            <div
              v-if="!contentStore.ourGentlemenLoaded"
            ...
            <div
              v-else-if="filteredGentlemen.length === 0"
              class="w-full !px-[16px] !py-[10px]"
            >
              No gentlemen available.
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
    filteredGentlemen() {
      if (!this.search) return this.contentStore.ourGentlemen;
      return this.contentStore.ourGentlemen.filter(
        (gentleman) =>
          gentleman.name.toLowerCase().includes(this.search.toLowerCase()) ||
          gentleman.igHandle.toLowerCase().includes(this.search.toLowerCase())
      );
    },
  },
  ...
  methods: {
    ...
    // Method to handle gentleman deletion
    deleteGentleman(gentlemanId, gentlemanPhotoUrl) {
      this.handleActions(gentlemanId);
      this.contentStore.confirmationCallback = () =>
        this.contentStore.deleteGentleman(gentlemanId, gentlemanPhotoUrl);
      ...
      this.contentStore.confirmationMessage =
        "Are you sure you want to delete this gentleman?";
      this.contentStore.confirmationActionText = "Delete";
    },

    // Method to handle gentleman editing
    editGentleman(gentlemanId) {
      this.handleActions(gentlemanId);
      this.$router.push({ name: "edit-gentleman", params: { id: gentlemanId } });
    },
  },
};
</script>
