    ...
    <div class="w-full flex flex-col gap-[24px] p-[24px]">
      ...
      <div class="w-full flex flex-col gap-[32px]">
        ...
        <div class="flex flex-col gap-[8px] w-full">
          <label class="font-semibold text-[14px] leading-[100%]" for="description">1. Review</label>
          <textarea rows="4" name="review" id="review" placeholder="Enter review here" v-model="review"
            autocomplete="off" required class="input"></textarea>
        </div>

        <!-- Reviewer -->
        <div class="flex flex-col gap-[8px] w-full">
          <label class="font-semibold text-[14px] leading-[100%]" for="title">2. Reviewer</label>
        ...
        <!-- Instagram Handle -->
        <div class="flex flex-col gap-[8px] w-full">
          <label class="font-semibold text-[14px] leading-[100%]" for="title">3. IG Handle</label>
      ...
      <div class="flex items-center gap-[16px] self-end">
        ...
        <button @click="updateReview" class="btn-prim px-[16px] py-[8px]">
          Update
        </button>
      </div>
    </div>
...
export default {
  ...
  async created() {
    ...
    try {
      ...
      if (!this.contentStore.reviews.length) {
        await this.contentStore.fetchReviews();
      }

      const matchedReview = this.contentStore.reviews.find(
        (review) => review.id === this.routeId
      );

      if (matchedReview) {
        this.matchedReview = matchedReview;
        this.selectedReviewId = matchedReview.id;
        this.review = matchedReview.review;
        this.reviewer = matchedReview.reviewer;
        this.igHandle = matchedReview.igHandle;
        this.reviewerPhotoUrl = matchedReview.reviewerPhotoUrl;
        this.currentFirebaseImageUrl = matchedReview.reviewerPhotoUrl;
      } else {
        this.contentStore.showStatusModal = true;
        this.contentStore.statusMessage = "The review was not found.";
  ...
  methods: {
    ...
    async updateReview() {
      ...
      if (this.matchedReview) {
        if (
          this.review === this.matchedReview.review &&
          this.reviewer === this.matchedReview.reviewer &&
          this.igHandle === this.matchedReview.igHandle &&
          this.reviewerPhotoUrl === this.matchedReview.reviewerPhotoUrl
        ) {
          this.contentStore.statusMessage =
      ...
      try {
        let reviewerPhotoUrl = this.reviewerPhotoUrl;
        // Prepare Firestore document
        const newReviewRef = doc(db, "reviews", this.selectedReviewId);
        ...
        this.currentFirebaseImageUrl = reviewerPhotoUrl;
        ...
        await withTimeout(updateDoc(newReviewRef, reviewerData));
        await this.contentStore.fetchReviews();
